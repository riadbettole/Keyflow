...
export function SecretRevealModal({ url, secret, onClose }: Props) {
		...
		<>
			...
			<div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md z-50 bg-gray-900 border border-gray-700 rounded-xl shadow-xl p-6">
				<div className="flex items-center gap-3 mb-5">
					...
					<div>
						<h2 className="text-gray-100 font-semibold text-base">Webhook created</h2>
						<p className="text-gray-400 text-xs mt-0.5 font-mono truncate max-w-xs">{url}</p>
					</div>
				</div>
				...
				<div className="bg-warning-muted border border-warning/20 rounded-lg px-4 py-3 mb-4">
					<p className="text-warning-text text-xs leading-relaxed">
						Copy your signing secret now — it will never be shown again. Use it to verify that
						webhook requests come from Keyflow.
					</p>
				</div>
				...
				<button
					onClick={onClose}
					className="w-full py-2 px-4 rounded-md text-sm font-medium bg-gray-800 hover:bg-gray-700 text-gray-200 transition-colors"
				>
